...
export const getDataTarifVersion = currentPage => {
  return dispatch => {
    let url = currentPage
      ? "/api/tarifversion?page=" + currentPage
      : "/api/tarifversion";
    ...
    axios
      ...
      .then(res => {
        ...
        dispatch(getTarifVersionSuccess(res.data.message));
      })
      .catch(err => {
        if (err.response) {
...
export const createDataTarifVersion = postData => {
  return dispatch => {
    axios
      .post("/api/tarifversion/", postData)
      .then(res => {
        console.log(res);
        // jika success
